...
- 이메일 가입 회원과 SNS(소셜 로그인) 가입 회원이 Email 이 겹치면 하나의 계정으로 자동 통합시킴.
- 인증 환경 세팅은 자주 변경됨
- 적용 시점에 다시 꼭 확인해야함.
...
- 로그인 → 앱 메뉴 → 애플리케이션 생성
- 생성된 앱 → Web 플랫폼 등록 → 사이트 등록 → `http://localhost:5173`

### 1.2. 반드시 Rest API 키를 보관

- `95d76f941be87ae67742ce6afe5d1b45`
...
- 카카오 로그인 메뉴 → 동의항목 → 개인정보 → ` 닉네임, 프로필 사진, 카카오 계정 (이메일)` 활성화

### 1.5. Redirect URI 등록

- 카카오 로그인 메뉴 → 일반 → 리다이렉트 URI
- 例 ) `http://localhost:5173/auth/callback` ( 추후 추가 설정 필요, 창 끄지 않기! )

### 1.6. Supabase 를 위한 `Client Secret` 설정 필요

- 카카오 로그인 메뉴 → 일반 → `Client Secret 발급 후 활성화`
- 시크릿 키 꼭 보관 : `T7iQG9JHYp0c3rHRg5JIAcgb4CRxLtvE`

## 2. Supabase 인증 세팅

- supabase → 프로젝트 → Authentication → Sign In / Providers 메뉴
- 스크롤 살짝 내리면 Auth Providers 항목에서 `kakao` Disabled 버튼 클릭
- Rest API Key 와 Client Secret 입력 (보관했던 키들 입력)
- Callback URL (for OAuth) 의 내용을 반드시 보관해 둬야함. (3번째 줄)
- `https://rqckhcqnpwvkjofyetzm.supabase.co/auth/v1/callback`
- 토글 Enabled 로 설정 후 Save.
- Callback URL 을 카카오 개발자의 Redirect URI 에 추가로 저장해둠.
- 그 이후 다시 Kakao 의 리다이렉트 URI 수정 버튼 클릭 후 해당 `https://rqckhcqnpwvkjofyetzm.supabase.co/auth/v1/callback` 붙여넣기.
...
- /src/contexts/AuthContext.tsx 카카오 로그인 기능 추가
...
- /src/pages/SignUpPage.tsx 수정 (업데이트)
...
- /src/pages/SignInPage.tsx 수정 (UI 업데이트)
...
- /src/pages/AuthCallback.tsx 수정 (카카오 로그인 후 프로필 업데이트)
...
## 5. 회원 가입 시 이메일 또는 카카오 이메일 중복 처리 방지

- 이메일 회원은 반드시 이메일 및 닉네임 중복 체크 후 회원가입 승인 진행

### 5.1 이메일과 닉네임 중복은 회원이 아닌 사람이 DB에 접근

- 웹에서 DB로 접근하는 것이 아니고, 바로 DB에 접근하는 코드가 필요함.
- DB 명령을 직접 실행하는 형태로 구성이 필요함 (많이 위험함 - 해킹위험이 있음)
...
### 5.4 제대로 PostgreSql Function 이 등록 되어있는지 확인
...
function SignUpPage() {
  ...
  const handleEmailCheck = async () => {
    ...
    if (!emailRegex.test(email)) {
      setEmailCheckMessage('올바른 이메일 형식을 입력 해주세요');
      setEmailCheckStatus('taken');
      return;
    }
    ...
    try {
      // DB 에 직접 이메일 글자를 보내고, 중복 확인을 진행함
  ...
  const handleNickNameCheck = async () => {
    if (!nickName.trim()) {
      setNickNameCheckMessage('닉네임을 입력 해주세요.');
      setNickNameCheckStatus('taken');
      return;
    }
    if (nickName.trim().length < 2) {
      setNickNameCheckMessage('닉네임은 2글자 이상 입력 해주세요.');
      setNickNameCheckStatus('taken');
      return;
    }
    ...
    try {
      ...
      setNickNameCheckMessage('닉네임 중복 확인 중 오류가 발생 했습니다.');
      setNickNameCheckStatus('taken');
    }
  };
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
              ...
              <button
                ...
                style={{
                  padding: '12px 16px',
                  backgroundColor: emailCheckStatus === 'available' ? '#10b981' : '#478cfb',
                  color: 'white',
                  border: 'none',
                  borderRadius: '4px',
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
              ...
              <button
                ...
                style={{
                  padding: '12px 16px',
                  backgroundColor: nickNameCheckStatus === 'available' ? '#10b981' : '#478cfb',
                  color: 'white',
                  border: 'none',
                  borderRadius: '5px',
...
- /src/contexts/AuthContext.tsx 업데이트 (이메일 중복 확인 기능 추가)
...
### 6.2. 시나리오 1번 : 카카오 회원 가입 후 이메일 회원 가입하기

### 6.3. 시나리오 2번 : 이메일 회원 가입 후 카카오 회원 가입하기

### 6.4. 시나리오 3번 : 관리자는 탈퇴를 못한다 (근데 이건 if 문 넣으면 끝아닌가)
...
const uploadAvatar = async (file: File, userId: string): Promise<string | null> => {
  try {
    ...
    // 기존에 만약 아바타 이미지가 있으면 무조건 삭제부터 한다.
...
const cleanupUserAvatars = async (userId: string): Promise<boolean> => {
  try {
    ...
    console.log(`아바타 이미지 전체삭제 오류 : ${error}`);
    return false;
  }
};
...
const removeAvatar = async (userId: string): Promise<boolean> => {
  try {
    // 현재 로그인한 사용자의 avatar_url 을 읽어와야 한다.
    // 여기서 파일명을 추출함.
    ...
    try {
      // url 의 파일명을 찾아야 함.(url 로 변환하면 path 와 파일구분 수월함)
      ...
      if (publicIndex !== -1 && publicIndex + 1 < pathParts.length) {
        ...
        if (error) {
          throw new Error('파일을 찾았지만, 삭제에는 실패했습니다.');
        }
        // 파일 삭제 성공
        deleteSuccess = true;
      }
