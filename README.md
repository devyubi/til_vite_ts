...
export interface DirectChat {
  ...
  user1_id: string; // 참여자 1번의 ID
  user2_id: string; // 참여자 2번의 ID
...
export interface CreateChatData {
  participant_id: string; // 상대방 참여자 ID
}
        ...
        return {
          ...
          last_message: lastMessage
            ? {
                ...
                sender_nickname:
                  lastMessage.sender_id === currentUser.id ? '유비' : otherUserInfo.nickname,
              }
            : undefined,
...
export async function searchUsers(searchTerm: string): Promise<ChatApiResponse<ChatUser[]>> {
  try {
    ...
    const { data: profiles, error: searchError } = await supabase
      ...
      .ilike('nickname', `%${searchTerm}%`) // sql 구문
      .limit(10);
...
const DrirectChatList = ({ onChatSelect, onCreateChat, selectedChatId }: DirectChatListProps) => {
  ...
  useEffect(() => {
    const subscription = supabase // direct_chats_changes 라는 이름으로 채널을 만든다
      .channel('direct_chats_changes')
      .on(
        'postgres_changes', // PostgreSQL 데이터 베이스의 변경사항을 알려주는 이벤트 명
        {
          event: '*', // 모든 이벤트 타입을 감지함 (insert, update, delete 등등등...)
          schema: 'public', // 스키마가 public 인 것이 대상임
          table: 'direct_chats', // 변경이 감시 되어질 테이블명
        },
        payload => {
          // 변경 사항에 대한 상세 정보 (새로운 데이터, 이전 데이터 등등등...)
          loadChats(); // 변경사항이 있을 때만 새로고침
        },
      )
      .subscribe(); // 구독을 신청함. (addEventListener 처럼)

    // 클린업 함수 : 컴포넌트가 언마운트 될 때 (화면에서 사라질 때) 실행
    return () => {
      // 구독 해제
