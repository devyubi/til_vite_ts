...
const newChatData: any = {
  name: '1:1 채팅', // 채팅방 이름
  type: 'direct', // 채팅방 타입
  partictipant_id: 'user-357', // 참여자 ID
};
```
...
- 어떤 데이터가 처리 후 API 응답시, 동일한 형태로 구성을 할 필요가 있음.
- 성공/실패 상태, 데이터/ERROR 를 구분해서 구성해야함
- 다양한 케이스 이므로, `제네릭`을 활용함
- 첫번째 : 하단은 샘플링 코드

```ts
// 성공 응답
const successResponse: any <데이터형 변경 가능> = {
  success : true,
  data: 데이터형에 맞는 데이터
}
// 실패 응답
const errorResponse: any <데이터형이 변경 가능> = {
  success: false,
  data: 'Error 에 대한 상세 내용'
}
```
...
export interface ChatApiResponse<T> {
  ...
  error?: string; // 에러 메세지 (실패시 - 선택적)
}
```

## 10. Mock 데이터를 위한 서비스 구성
 ...
 *  - 에러처리도 상세하게 진행
 *  - 실시간 기능 (WebSocket 또는 Supabase RealRimes 추가)
 */
...
const mockUser: ChatUser[] = [
  ...
  { id: '2', email: 'user2@example.com', nickname: '홍길동', avatar_url: null },
  { id: '3', email: 'user3@example.com', nickname: '고영희', avatar_url: null },
 ...
 * Mock 채팅방 목록 데이터
 *  - 실제로는 Supabase 의 ??? (추후 결정) 테이블에서 가져올 데이터 샘플
 *  - 채팅방 ID 를 Key로 해서 각 채팅방의 메세지를 그룹화 후 관리
 */
...
- directChatService.ts 업데이트

```ts
/**
 * 1:1 채팅방 생성 또는 찾기
 *  - 사용자가 특정 사용자와 채팅ㄹ을 시작하려고 할 때 호출
...
export async function findOrCreateDirectChat(
  ...
  const newChat: Chat = {
    ...
    created_at: new Date(Date.now() - 1000 * 60 * 30).toISOString(), // 채팅방 생성시간
    updated_at: new Date(Date.now() - 1000 * 60 * 30).toISOString(), // 마지막 업데이트 시간
  };
...
export async function getMessages(chatId: string): Promise<ChatApiResponse<MessageDetail[]>> {
  await new Promise(resolve => setTimeout(resolve, 500));
  // Mock 데이터로서 해당 채팅방의 메세지들을 전체 조회
...
const DrirectChatList = ({ onChatSelect, onCreateChat, selectedChatId }: DirectChatListProps) => {
  ...
  useEffect(() => {
    // 만약 사용자 검색어가 있다면
    if (searchTerm.trim()) {
      // console.log('DB에서 사용자 닉네임을 실시간 검색 함 ...');
      // 검색어가 입력이 되면, service의 사용자 검색 API 를 호출해야함
      searchUsers(searchTerm);
    }
  }, [searchTerm, searchUsers]);

  // 날짜 관련 포맷 설정 - 추후에 util 처럼 계속 재사용하면 됨
  ...
  return (
    <div className="chat-list">
      ...
      <div className="chat-list-header">
        <h2>1 : 1 채팅</h2>
        {/* 사용자가 새로운 채팅을 생성시 showUserSearch 를 true 로 변경 */}
              ...
              <div key={user.id} className="user-item" onClick={() => handleUserSelect(user)}>
                ...
                <div className="user-avatar">
                  {user.avatar_url ? (
                    ...
                    // 사용자 아바타 닉네임 출력 (이미지가 없을 경우 - 닉네임의 첫 글자만 보여줌)
              ...
              <div className="chat-info">
                ...
                <div className="chat-preview">
                  {chat.last_message ? (
                    ...
                    <span className="no-message">메세지가 없습니다.</span>
                  )}
                </div>
              </div>
            </div>
...
export default DrirectChatList;
```
